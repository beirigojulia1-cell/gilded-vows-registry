        ...
        <div className="p-7 md:p-10">
          {done ? (
            <div className="text-center py-10">
              ...
              <h3 className="font-serif text-3xl text-gradient-gold mb-2">Pagamento confirmado!</h3>
            ...
            <>
              ...
              <div className="flex border border-gold/25 rounded overflow-hidden mb-5">
                <button
                  type="button"
                  onClick={() => setTab("pix")}
                  className={`flex-1 py-2.5 text-[0.7rem] tracking-[0.2em] uppercase transition-colors ${tab === "pix" ? "bg-gold text-ink" : "text-champagne/70 hover:text-gold"}`}
                >
                  PIX
                </button>
                <button
                  type="button"
                  onClick={() => setTab("card")}
                  className={`flex-1 py-2.5 text-[0.7rem] tracking-[0.2em] uppercase transition-colors ${tab === "card" ? "bg-gold text-ink" : "text-champagne/70 hover:text-gold"}`}
                >
                  Cartão de Crédito
                </button>
              </div>
              ...
              {tab === "pix" ? (
                pix ? (
                  <div className="border border-gold/20 rounded-lg p-5 bg-black/30 space-y-4">
                    <p className="text-[0.65rem] tracking-[0.25em] uppercase text-gold/70 text-center">Escaneie o QR Code ou copie o código</p>
                    {pix.qrBase64 ? (
                      <img src={`data:image/png;base64,${pix.qrBase64}`} alt="QR Code PIX" className="mx-auto w-56 h-56 rounded bg-white p-2" />
                    ) : null}
                    <div className="flex gap-2">
                      <code className="flex-1 px-3 py-2 bg-black/40 border border-border rounded text-champagne/80 text-xs truncate">{pix.qrCode}</code>
                      <button
                        type="button"
                        onClick={copyPix}
                  ...
                  <form onSubmit={startPix}>
                    <button type="submit" disabled={loading} className="shimmer w-full py-3 rounded bg-gradient-to-r from-[#8a6f3d] via-[#c9a96e] to-[#8a6f3d] text-ink font-medium tracking-[0.18em] uppercase text-xs disabled:opacity-50">
                ...
                <form onSubmit={startCard}>
                  <button type="submit" disabled={loading} className="shimmer w-full py-3 rounded bg-gradient-to-r from-[#8a6f3d] via-[#c9a96e] to-[#8a6f3d] text-ink font-medium tracking-[0.18em] uppercase text-xs disabled:opacity-50">
                    {loading ? "Redirecionando…" : "Pagar com cartão de crédito"}
                  </button>
                  <p className="text-center text-champagne/50 text-[0.65rem] mt-3">Você será redirecionado para o checkout seguro do Mercado Pago · Parcele em até 12x</p>
                </form>
              )}
            </>
          )}
